...
class Filters extends Component
{
    ...
    // set images paths depends on filter
    ...
    public function mount($post_caption, $image_path) // to set image path and post_caption
    {
    ...
    public function applyFilter($num) // to save the image with the filter
    {
        // replace the old image with the new filtered image
        switch ($num) {
            case 0:
                break;
            case 1:
    ...
    // argnum to know how many args are passed
    public function correctImageOrientation($imagepath, $effect, $newimagepath, $arg1, $arg2, $arg3, $arg4, $argnum) // correct the orientation of the image after storing it in jpeg format using imagejpeg()
    {
        $img = imagecreatefromstring(file_get_contents('storage/' . $imagepath)); // read the image

        // exif_read_data reads the EXIF headers from an image file. This way you can read meta data generated by digital cameras.
        if (@exif_read_data('storage/' . $imagepath)) { // if the image has the data
            $exif = @exif_read_data('storage/' . $imagepath);

            // if the image has the Orientation field and exif data
            if ($exif && isset($exif['Orientation'])) {
                $orientation = $exif['Orientation'];

                if ($orientation != 1) {
                    $deg = 0;
                    // orientation of an image is 3 or 6 or 8
                    switch ($orientation) {
                        case 3:
                            $deg = 180;
                    ...
                    //to set imagefilter arguments count
                    switch ($argnum) {
                        case '0':
                    ...
                    imagejpeg($img, $newimagepath, 100); // store in jpeg format
                    imagedestroy($img); // destroy the variable in the memory
                }

                // else if the image doest have Orientation field
            } else {
                //to set imagefilter arguments count
                switch ($argnum) {
                    case '0':
            ...
            // else if the image doesnt have EXIF data
        } else {
            //to set imagefilter arguments count
            switch ($argnum) {
                case '0':
